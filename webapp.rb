  ...
  helpers do
    ...
    def tweets(params={})
      @tweets = DB["select tweets.*, twitter_users.* from tweets inner join twitter_users using (twitter_user_id)
          inner join sourcings on tweets.tweet_id = sourcings.tweet_id
          inner join sources on sources.source_id = sourcings.source_id
          where sources.app_id = ?", app_id].
